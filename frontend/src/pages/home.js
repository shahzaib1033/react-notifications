...
function Home() {
    ...
    const allownotifications = () => {
        console.log("working with allownotifications");
        setupNotifications((message) => {
            const { title, body } = message;
            if (isForeground) {
                toastNotification({
    ...
    return (
        <div className="App">
            ...
            {/* {token&&<button onClick={allownotifications}>Regenerate Token</button>} */}
            {token && <button onClick={() => { navigate("/test-notifications") }}>Check How its working</button>}
